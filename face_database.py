...
FACE_YUNET_MODEL = os.path.join(
    BASE_DIR,
    "models",
    "face_detection_yunet_2023mar.onnx"
)

FACE_SFACE_MODEL = os.path.join(
    BASE_DIR,
    "models",
    "face_recognition_sface_2021dec.onnx"
)

FACE_EMBEDDINGS_FILE = os.path.join(
    BASE_DIR,
    "data",
    "face_embeddings.pkl"
)
...
class CameraSession:
    ...
    # Latest camera frame kept separate so the live stream never has to wait
    # for YOLO/face analytics to finish.
    live_frame: object = None
    ...
    person_identity: dict = field(

        default_factory=dict

    )

    person_identity_similarity: dict = field(

        default_factory=dict

    )

    person_face_last_check: dict = field(

        default_factory=dict

    )

    person_face_last_seen: dict = field(
...
def process_frame(
    ...
    for candidate in vehicle_candidates:
        ...
        db_display = vehicle_info.get("display_name")
        label = (
            f"{db_display} "
            if db_display else
            f"{class_name} "
    ...
    while session.active:
        ...
        try:
            ...
            with session.frame_lock:
                ...
                session.processed_timestamp = time.time()
...
def main():
    ...
    face_system_ready = load_face_recognition_system()

    if face_system_ready:
