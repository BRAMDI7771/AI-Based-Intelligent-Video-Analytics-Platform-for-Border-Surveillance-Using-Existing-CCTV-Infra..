...
def process_frame(
    ...
    for candidate in person_candidates:
        ...
        # 1. Person crop nikalna
person_crop = frame[int(y1):int(y2), int(x1):int(x2)]

# 2. Face identification call
name = identify_face(person_crop)
# 3. Label text decide karna
if name != "UNKNOWN":
        display_text = f"{name} (ID:{track_id})"
        box_color = (0, 255, 0) # Identified person ke liye Green Box
else:
        display_text = f"PERSON ID:{track_id}"
        box_color = (0, 0, 255) # Unknown ke liye Red Box
# 4. Bounding Box Draw Karna (Rectangle)
cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), box_color, 2)
cv2.putText(frame, display_text, (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, box_color, 2)
session.previous_boxes[track_id] = (int(x1), int(y1), int(x2), int(y2))
...
# SYSTEM STATUS
# ========================================================
cv2.putText(output, "SYSTEM STATUS: MONITORING", (20, max(25, h - 18)), cv2.FONT_HERSHEY_SIMPLEX, 0.60, (0, 255, 0), 2)
    cv2.putText(output, display_text, (20, max(25, h - 45)), cv2.FONT_HERSHEY_SIMPLEX, 0.60, (0, 255, 0), 2)
