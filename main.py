...
def process_frame(
    ...
    # Encode exactly once per processed frame. Dashboard clients reuse this
    # JPEG instead of re-encoding the same frame and building a browser backlog.
    try:
        ...
        if ok_jpeg:
            with session.frame_lock:
                session.latest_jpeg = jpeg_buf.tobytes()
                session.latest_jpeg_count = session.processed_count + 1
    except Exception:
        pass
